 ...
 *
 * This file contains setup routines for the OpenGL context, GLFW
 * initialization, mesh loading, shader compilation, and the rendering main
 * loop. It connects all key graphics components and allows for dynamic window
 * resizing.
 */
...
/**
 * @brief Initializes OpenGL, loads scene data, sets up rendering objects, and
 * runs the main render loop.
 *
 * Creates the OpenGL context and window, loads vertex and edge data from the
 * provided ObjParser, compiles shaders, constructs for the mesh, transformation
 * matrices, and renderer. Handles window resizing and entire application
 * lifecycle.
 *
...
int runRenderer(const ObjParser &parser, const std::string &windowTitle) {
  ...
  Shader shader("gpu_wireframing/shaders/advanced/vertex_shader.glsl",
                "gpu_wireframing/shaders/advanced/fragment_shader.glsl");
  ...
  viewMatrix.setCamera(MiniGLM::vec3(0, 0, 10), MiniGLM::vec3(0, 0, 0),
                       MiniGLM::vec3(0, 1, 0));
  ...
  Renderer renderer(window, mesh, shader, modelMatrix, viewMatrix,
                    projectionMatrix);
...
/**
 * @brief GLFW callback for updating the viewport and renderer state on window
 * resize.
 *
