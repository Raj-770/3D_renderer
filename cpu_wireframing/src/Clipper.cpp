...
int Clipper::computeOutCode(int x, int y) const {
    int code = INSIDE;
    if (x < xmin_) code |= LEFT;
    else if (x > xmax_) code |= RIGHT;
    if (y < ymin_) code |= BOTTOM;
    else if (y > ymax_) code |= TOP;
    return code;
}

MiniGLM::vec4 Clipper::lerp(const MiniGLM::vec4& a, const MiniGLM::vec4& b, float t) const {
    return MiniGLM::vec4(
        a.x + t * (b.x - a.x),
        a.y + t * (b.y - a.y),
        a.z + t * (b.z - a.z),
...
bool Clipper::clipLineNearPlane(MiniGLM::vec4& p0, MiniGLM::vec4& p1) const {
    ...
    // For standard perspective projection, nearPlane is typically the near distance
