...
ViewMatrix::ViewMatrix()
    : position_(0.0f, 0.0f, 10.0f), target_(0.0f, 0.0f, 0.0f),
      up_(0.0f, 1.0f, 0.0f), dirty_(true) {}
 ...
 */
void ViewMatrix::setCamera(const MiniGLM::vec3 &position,
                           const MiniGLM::vec3 &target,
                           const MiniGLM::vec3 &up) {
  position_ = position;
  target_ = target;
  up_ = up;
  dirty_ = true;
...
/**
 * @brief Returns the camera's view matrix, recalculation if camera state has
 * changes
 *
 * @return MiniGLMmat4
 */
...
/**
 * @brief Recomputes the view matrix using lookAt with current camera
 * parameters.
 *
 * Applies MiniGLMlookAt to construct the view matrix from the camera's positon,
